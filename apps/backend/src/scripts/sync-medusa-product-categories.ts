...
/**
 * Assign Medusa product categories from website folder taxonomy (collapsed).
 * Skips deep leaf handles — products link to max depth 1 for now.
 *
 * Usage:
 *   medusa exec ./src/scripts/sync-medusa-product-categories.ts
 *   medusa exec ./src/scripts/sync-medusa-product-categories.ts -- --dry-run
 *   medusa exec ./src/scripts/sync-medusa-product-categories.ts -- --manufacturer=zenitel
 */
export default async function syncMedusaProductCategories({
  container,
  args,
}: Args) {
  ...
  logger.info(
    `[medusa-cat] sync start dryRun=${dryRun} manufacturer=${only ?? "all"} website=${websiteIndex.size} maxDepth=1`
  )
  ...
  const targets = (products as any[]).filter((product) => {
    const mid = inferManufacturerId({
      handle: product.handle,
      manufacturerId: product.metadata?.manufacturer_id,
      manufacturer: product.metadata?.manufacturer ?? product.metadata?.brand,
    })
    if (!mid) {
      return false
    }
  ...
  for (const product of slice) {
    ...
    sourceCounts.set(leafHandle, (sourceCounts.get(leafHandle) ?? 0) + 1)

    const assignmentHandle = medusaCategoryHandle(
      leafHandle,
      1,
      handleIndex
    )
    ...
    if (assignmentHandle !== leafHandle) {
      stats.collapsed += 1
    }
    ...
    if (!categoryId) {
      ...
      logger.warn(
        `[medusa-cat] unknown handle "${assignmentHandle}" (from ${leafHandle}) for ${product.handle}`
      )
      continue
    }
    ...
    targetCounts.set(
      assignmentHandle,
      (targetCounts.get(assignmentHandle) ?? 0) + 1
    )
  ...
  logger.info(
    `[medusa-cat] plan resolved=${stats.resolved} uncategorized=${stats.uncategorized} collapsed=${stats.collapsed} skipped=${stats.skipped} legacy=${stats.legacy} unchanged=${stats.unchanged} missing=${stats.missingHandle} toUpdate=${updates.length}`
  )

  const topTargets = [...targetCounts.entries()]
    .sort((a, b) => b[1] - a[1])
    .slice(0, 20)
  for (const [handle, count] of topTargets) {
    logger.info(`  medusa ${handle}: ${count}`)
  }

  if (dryRun) {
    logger.info("[medusa-cat] dry-run complete — no writes")
  ...
  for (let i = 0; i < updates.length; i += batchSize) {
    ...
    try {
      ...
      logger.info(`[medusa-cat] updated ${stats.updated}/${updates.length}`)
    } catch (error) {
      stats.errors += batch.length
      logger.error(
        `[medusa-cat] batch failed at offset ${i}: ${
      ...
      for (const item of batch) {
        try {
          ...
          logger.error(
            `[medusa-cat] item failed ${item.id}: ${
  ...
  logger.info(`[medusa-cat] done updated=${stats.updated} errors=${stats.errors}`)
  return stats
}
