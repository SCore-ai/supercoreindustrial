import { medusaCookieOptions } from "@lib/cookie-options"
...
export async function middleware(request: NextRequest) {
  ...
  if (urlHasCountry) {
    const preview =
      request.nextUrl.searchParams.get("online_store_preview") === "1"
    const token = request.nextUrl.searchParams.get("token")

    const requestHeaders = new Headers(request.headers)
    if (preview && token) {
      requestHeaders.set("x-online-store-preview", "1")
      requestHeaders.set("x-online-store-preview-token", token)
    }
